...
Ceph一个主要伸缩功能就是避免了对象存储的中央接口，这就要求Ceph客户端能直接和OSD交互。Ceph通过cephx认证系统保护数据，它也认证运行Ceph客户端的用户，cephx协议运行机制类似Kerberos（关于Kerberos，请参见[https://web.mit.edu/kerberos/](https://web.mit.edu/kerberos/ "Kerberos网络认证协议")）。
